...
def compute_loss(X, y, theta):
    ...
    prediction_vector = np.dot(X, theta) # compute y_hats
    prediction_vector = prediction_vector - y # compute loss per instance
    prediction_vector = prediction_vector ** 2
    J = (1 / (2 * n)) * np.sum(prediction_vector) #compute mean
...
def find_best_learning_rate(X_train, y_train, X_val, y_val, iterations):
    ...
    for eta in etas:
        ...
        theta, J_history = gradient_descent_stop_condition(X_train, y_train, np.random.randn(X_train.shape[1]), eta, iterations)
